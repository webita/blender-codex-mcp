...
logger = logging.getLogger("BlenderCodexMCPServer")
...
async def server_lifespan(server: FastMCP) -> AsyncIterator[Dict[str, Any]]:
    ...
    try:
        ...
        logger.info("Blender Codex MCP server starting up")
        ...
        logger.info("Blender Codex MCP server shut down")
...
def _run_blender_python(code: str) -> dict[str, Any]:
    """Execute Python code in Blender and return the raw result dict."""
    blender = get_blender_connection()
    return blender.send_command("execute_code", {"code": code})
...
def execute_blender_code(ctx: Context, code: str) -> str:
    ...
    """
    try:
        result = _run_blender_python(code)
...
@telemetry_tool("blender_health_check")
@mcp.tool()
def blender_health_check(ctx: Context) -> str:
    """
    Verify that the Blender bridge is reachable and report basic session information.
    """
    try:
        blender = get_blender_connection()
        ...
        payload = {
            ...
            "host": blender.host,
            "port": blender.port,
            "scene_name": scene_result.get("name"),
            "object_count": scene_result.get("object_count"),
            "materials_count": scene_result.get("materials_count"),
            "preview_objects": scene_result.get("objects", []),
        }
...
@telemetry_tool("sync_camera_to_viewport")
@mcp.tool()
def sync_camera_to_viewport(ctx: Context) -> str:
    """
    Move the active scene camera to match the current 3D viewport view.
    Requires an open VIEW_3D area in Blender.
    """
    code = """
import bpy
...
if view3d_area is None or window_region is None:
    raise RuntimeError("No VIEW_3D area with WINDOW region found")

with bpy.context.temp_override(area=view3d_area, region=window_region, space=space):
    space.region_3d.view_perspective = 'CAMERA'
    bpy.ops.view3d.camera_to_view()

print("camera_synced", camera.name, tuple(round(v, 4) for v in camera.location), tuple(round(v, 4) for v in camera.rotation_euler))
"""
    try:
        result = _run_blender_python(code)
        return f"Camera synced successfully: {result.get('result', '')}"
    except Exception as e:
        logger.error(f"Error syncing camera: {str(e)}")
        return f"Error syncing camera: {str(e)}"
...
def export_glb(
    ...
    """
    Export the current Blender scene or the selected objects to a GLB file.

    Parameters:
    - output_path: Absolute or relative path to the .glb file to write
    - export_selected: Export only the selected objects when true
    - apply_modifiers: Apply modifiers during export when true
    """
...
bpy.ops.export_scene.gltf(
    ...
    export_format='GLB',
    use_selection={export_selected!r},
    export_apply={apply_modifiers!r},
)

print("glb_exported", output_path)
"""
    try:
        result = _run_blender_python(code)
        return f"GLB exported successfully to {output_path}. {result.get('result', '')}".strip()
    except Exception as e:
        logger.error(f"Error exporting GLB: {str(e)}")
        return f"Error exporting GLB: {str(e)}"
...
def get_polyhaven_categories(ctx: Context, asset_type: str = "hdris") -> str:
    ...
    try:
        ...
        if not _polyhaven_enabled:
            return "PolyHaven integration is disabled. Select it in the Blender Codex MCP sidebar, then run it again."
